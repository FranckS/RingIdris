...
The hierarchy of interfaces continues with commutative-monoids, groups, commutative groups, semi-rings and rings, but we only show the \code{Group} interface for reasons of space :
...
When an expression is a variable \code{(VarMa\ \_\ $\Gamma$\ i)}, the denoted number is the corresponding variable in the context, ie, \code{(index\ i\ $\Gamma$)}.
...
\item The top-level sum of the polynomial will be in right-associative form : \\
$prodMon_1\ +\ (prodMon_2\ + (prodMon_3\ +\ (...\ +\ prodMon_a)))$
\item All the products that we have at the two levels, namely the products of monomials (at the main level) and the products of variables (at the monomial level), will also be written on the completely right-associative form.
