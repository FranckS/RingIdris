...
exprMo_eq p (ConstMo p const1 b1) (ConstMo p const2 b2) with ((monoid_eq_as_elem_of_set p) const1 const2, decEq b1 b2)
    exprMo_eq p (ConstMo p const1 b1) (ConstMo p const1 b1) | (Just refl, Yes refl) = Just refl -- Attention, the clause is with "Just refl", and not "Yes refl"
    exprMo_eq p (ConstMo p const1 b1) _ | _ = Nothing
exprMo_eq p _ _  = Nothing

total
elimZero : (c:Type) -> (p:dataTypes.Monoid c) -> {g:Vect n c} -> {c1:c} -> (ExprMo p g c1) -> (c2 ** (ExprMo p g c2, c1=c2))
elimZero c p (ConstMo p const b1) = (_ ** (ConstMo p const b1, refl))
-- elimZero c p (PlusMo (ConstMo p const1) (VarMo p v)) = ?foo --with (monoid_get_setEq p Zero const1) 
elimZero c p (PlusMo (ConstMo p const1 b1) (VarMo p v b)) with (monoid_eq_as_elem_of_set p Zero const1)
...
--  elimZero c p (PlusMo (ConstMo p const1) (VarMo p v)) | (Just prf) = ?elimZero_rhs_2
    elimZero c p (PlusMo (ConstMo p dataTypes.Zero b1) (VarMo p v b)) | (Just refl) = (_ ** (VarMo p v b, ?MelimZero1))
    elimZero c p (PlusMo (ConstMo p const1 b1) (VarMo p v b)) | _ = (_ ** (PlusMo (ConstMo p const1 b1) (VarMo p v b), refl)) 
elimZero c p (PlusMo (VarMo p v b) (ConstMo p const2 b2)) with (monoid_eq_as_elem_of_set p Zero const2) 
    elimZero c p (PlusMo (VarMo p v b) (ConstMo p dataTypes.Zero True)) | (Just refl) = (_ ** (VarMo p v b, ?MelimZero2))
    elimZero c p (PlusMo (VarMo p v b) (ConstMo p const2 b2)) | _ = (_ ** (PlusMo (VarMo p v b) (ConstMo p const2 b2), refl))
