...
\begin{lstlisting}[caption=Build proof, captionpos=b, label=lst1:haskell2]
...
\end{lstlisting}
Now, we need to define the function reduce. To do that, we have to fix a canonical representation of associative lists. We decide that the right associative form will be the canonical representation. Thus, the $reduce$ function will have to rewrite the metaified term by rearranging the parentheses in order to transform it in the form $l1 ++ (l2 ++ (l3 ++(... ln)))$. To do so, we can define a new datatype which captures this property. Thus it will be easier to get sure that we are effectively computing the normal form : forcing properties to hold by the shape of a datatype is a good usage of depend types when, like here, it doesn't introduce many complications.
\begin{lstlisting}[caption=New reflected lists, captionpos=b, label=lst1:haskell2]
  data RExpr : (G : Vect n (List a)) -> 
                List a -> Type where
       RApp : RExpr G x -> (i : Fin n) -> 
              RExpr G (x ++ index i G)
       RNil : RExpr G []
\end{lstlisting}
This datatype has only two constructors. In fact, it combines the previous $Var$ and $App$ constructors so that it becomes impossible to write an expression which isn't right associative.
