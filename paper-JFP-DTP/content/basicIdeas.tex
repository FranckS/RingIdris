...
The goal of the normalization is of course to compute a canonical representation for a number $x$, such that any other number provably equal to $x$ (by using the two available properties) will have the same canonical representation. For example, the normalisation might transform $x+((y+Z)+z)$ into $(x+y)+z$ if we decide that the normalisaiton form will be the complete left associative form. It will then be possible to decide the equality by simply comparing the normalised left and right hand sides with a simple syntactical equality. \\
\\
In fact, such a normalisation function can't be written, because in the LHS and RHS of $x=y$, we potentially have variables which have been universally quantified. And the normalization function needs to do different treatments for a "variable natural number" (a number which has been universally quantified) and for the constant $Z$. This is not possible yet, because once the variables are abstracted, they are just normal values of type $Nat$, and nothing tells us where they are in the left and right hand side of the equality : this information only exists at the level of the ASTs representing the two terms.
For this reason, we will work by reflection, as we will do for the hierarchy of tactics in the next section. In this smaller example, it means that we will define a datatype which will be used as an encoding of natural numbers, or more precisely, an encoding of natural numbers composed of "variable numbers", $Z$, and additions of these. This datatype will allow us to know the internal structure of a number, ie, where the variables and constants are.
...
\end{verbatim}
\end{center}
\caption{First version of reflected numbers}
\figrule
\end{figure}
...
where $reify$ is a function computing the interpretation of an Expr in a context $\Gamma$, that is to say, the number that this Expr is encoding.
...
\\
To avoid these two sources of complexity, we add an index to the type $Expr$ which is the concrete number that an Expr is encoding. Thus, it won't be necessary to define the $interpretation$ function. \\
...
\\
We have an expression $e1$ encoding $x$, and an expression $e2$ encoding $y$\footnote{These encodings have to be produced by hand by the user for this small simple problem, but for the real collection of tactics, we will program an automatic metaification}.
